class UserTasksController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @user_task.save
        ...
        # looks for a partial called the same thing as the name of the method
        format.js {}
        format.json { render :show, status: :created, location: @user_task }
      else
        format.html { render :new }
        # looks for a partial with the name 'new'
  ...
  private
    ...
    def all_tasks
      @user_tasks = UserTask.where(user_id: current_user.id).page(params[:page]).order(:due)
    end
